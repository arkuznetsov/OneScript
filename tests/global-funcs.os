...
Функция ПолучитьСписокТестов(ЮнитТестирование) Экспорт
	...
	ВсеТесты.Добавить("Тест_ДолженПроверитьФункциюОкр");
	...
	ВсеТесты.Добавить("Тест_ДолженПроверитьФункциюСимвол");
	ВсеТесты.Добавить("Тест_ДолженПроверитьНеобязательныйПараметрФункцииКодСимвола");
	ВсеТесты.Добавить("Тест_ДолженПроверитьПараметрФункцииКодСимволаВнеСтроки");
...
Процедура Тест_ДолженПроверитьФункциюОкр() Экспорт

	юТест.ПроверитьРавенство( Окр(1234.567), 1235);
	юТест.ПроверитьРавенство( Окр(2234.567,2), 2234.57);
	юТест.ПроверитьРавенство( Окр(3234,-2), 3200);
	юТест.ПроверитьРавенство( Окр(4234.567,-2), 4200);
	
	юТест.ПроверитьРавенство( Окр(5234.55,1), 5234.6);
	юТест.ПроверитьРавенство( Окр(6234.55,1,0), 6234.5);
	юТест.ПроверитьРавенство( Окр(7234.55,1,1), 7234.6);
	юТест.ПроверитьРавенство( Окр(8234.555,1,0), 8234.6);
	юТест.ПроверитьРавенство( Окр(9234.55,1,-4), 9234.6);
	
	юТест.ПроверитьРавенство( Окр(10250,-2,0), 10200);
	юТест.ПроверитьРавенство( Окр(11251,-2,0), 11300);
	юТест.ПроверитьРавенство( Окр(12251,-2,1), 12300);
	юТест.ПроверитьРавенство( Окр(-13250,-2,0), -13200);
	юТест.ПроверитьРавенство( Окр(-14251,-2,0), -14300);
	юТест.ПроверитьРавенство( Окр(-15250,-2,1), -15300);

	юТест.ПроверитьРавенство( Окр(-16234.55,1,0), -16234.5);
	юТест.ПроверитьРавенство( Окр(-17234.55,1,1), -17234.6);
	юТест.ПроверитьРавенство( Окр(-18234.551,1,0), -18234.6);
	юТест.ПроверитьРавенство( Окр(-19234.551,1,1), -19234.6);

	юТест.ПроверитьРавенство( Окр("20234.5","0",-1.9), 20235);
...
Процедура Тест_ДолженПроверитьФункциюСимвол() Экспорт

	Строка = Символ(1040);
	юТест.ПроверитьРавенство(Строка, "А");

	Строка = Символ(-2);
	юТест.ПроверитьРавенство(Строка, "", "код < 0");
	Строка = Символ(65859);
	юТест.ПроверитьРавенство(Строка, "", "код > 65536");
...
Процедура Тест_ДолженПроверитьПараметрФункцииКодСимволаВнеСтроки() Экспорт

	Строка = "АБВ";
	юТест.ПроверитьРавенство(-1, КодСимвола(Строка,0), "номер символа = 0");
	юТест.ПроверитьРавенство(-1, КодСимвола(Строка,-3), "номер символа < 0");
	юТест.ПроверитьРавенство(-1, КодСимвола(Строка,7), "номер символа > длины строки");
	
	Строка = "";
	юТест.ПроверитьРавенство(-1, КодСимвола(Строка), "пустая строка, без параметра");
	юТест.ПроверитьРавенство(-1, КодСимвола(Строка,0), "пустая строка, номер символа = 0");
	юТест.ПроверитьРавенство(-1, КодСимвола(Строка,-1), "пустая строка, номер символа < 0");
	юТест.ПроверитьРавенство(-1, КодСимвола(Строка,1), "пустая строка, номер символа > 0");
...
Процедура Тест_ДолженПроверитьДекодированиеURL() Экспорт
	...
	// Проверить передачу Неопределено и Null (не должно упасть)
	РаскодироватьСтроку(Неопределено, СпособКодированияСтроки.КодировкаURL);
    РаскодироватьСтроку(Null, СпособКодированияСтроки.КодировкаURL);
