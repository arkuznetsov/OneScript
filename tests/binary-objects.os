...
Процедура ТестДолжен_ПроверитьЧтоПрочитатьБайтНеВыбрасываетИсключениеВКонцеПотока() Экспорт
    
    Буфер = ПолучитьБуферСДанными();
    Буфер.УстановитьТолькоЧтение();
    Поток = Новый ПотокВПамяти(Буфер);
    ЧтениеДанных = Новый ЧтениеДанных(Поток.ЗакрытьИПолучитьДвоичныеДанные());
    
    Сч = 0;
    Пока Сч <= Буфер.Размер Цикл // читаем после конца буфера
    
        Байт = ЧтениеДанных.ПрочитатьБайт();
        Если Байт = Неопределено Тогда
            Прервать; // должны зайти сюда
        КонецЕсли;
